...
#define MAX_SERIAL_CHANNELS 3

static int nSerial = 0;
static Serial * serialChannels[MAX_SERIAL_CHANNELS];
...
Serial * createSerial(UART_HandleTypeDef * pHandle, 
	char * inputBuffer,
...
{
	Serial * result;
	result = new Serial(pHandle, inputBuffer, sizeInput, outputBuffer, sizeOutput);
	serialChannels[nSerial++] = result;

	return result;

}
Serial::Serial(UART_HandleTypeDef * pHandle, 
				char * inputBuffer, unsigned int sizeInput, 
				char * outputBuffer, unsigned int sizeOutput)
{
	_pHandle = pHandle;
	input = SerialInput(pHandle, inputBuffer, sizeInput);
	output = SerialOutput(pHandle, outputBuffer, sizeOutput);
...
{
	for (int i=0; i < MAX_SERIAL_CHANNELS; i++) {
		if (serialChannels[i]->_pHandle == UartHandle)
			serialChannels[i]->input.doInputIT();
		return;
	}
	Error_Handler();
}
...
{
	for (int i; i < MAX_SERIAL_CHANNELS; i++) {
		if (serialChannels[i]->_pHandle == UartHandle)
			serialChannels[i]->output.doOutputIT();
		return;
	}
	Error_Handler();
}
